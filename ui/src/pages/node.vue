...
const pushToApp = (item, app) => {
  ...
  if (app === 'file') {
    router.push('/node/file/' + selectedItem.value['id'])
    return;
  }

  if (app === 'shortcut') {
    router.push('/node/shortcut/' + selectedItem.value['id'])
  }
}
const backToMain = () => {
  ...
  requestAnimationFrame(() => {
    ...
    topBarStyle.value.animation = 'screen-up 0.66s reverse';
  });
...
const menuOptions = [
  {
    label: () => h(
        RouterLink,
        {
          to: '/node/file/' + selectedItem.value['id'],
          'class': 'ml-2'
        },
        {
          default: () => '文件'
  ...
  },
  {
    label: () => h(
        RouterLink,
        {
          to: '/node/shortcut/' + selectedItem.value['id'],
          'class': 'ml-2'
        },
        {
          default: () => '捷径'
